...
public class WifiSession implements Runnable {
    ...
    // constructor
    public WifiSession(@NonNull ForegroundService context, int interval) {
    ...
    public void startSession(final String streamFolder) {
        ...
        if (!mWifiManager.isWifiEnabled()) {
            Log.d(LOG_TAG, "startSession: Please turn on Wifi first!");
            return;
        }
        ...
        if (streamFolder != null) {
            try {
                ...
                Log.d(LOG_TAG, "startSession: Cannot create file for Wifi scans");
                e.printStackTrace();
            }
        }
        run();
