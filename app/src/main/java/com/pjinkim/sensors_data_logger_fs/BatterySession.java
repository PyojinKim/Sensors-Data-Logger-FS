...
public class BatterySession implements Runnable {
    ...
    private final static int DEFAULT_INTERVAL = 30000; // milli second
    ...
    // constructor
    public BatterySession(@NonNull ForegroundService context, int interval) {
        this.mContext = context;
        this.mBatteryInterval = interval;
        mBatteryManager = (BatteryManager) mContext.getSystemService(Context.BATTERY_SERVICE);
    }

    BatterySession(@NonNull ForegroundService context) {
    ...
    public void startSession(final String streamFolder) {
        ...
        if (streamFolder != null) {
            try {
                ...
                Log.d(LOG_TAG, "startSession: Cannot create file for battery level.");
                e.printStackTrace();
            }
        }
        run();
    ...
    class BatteryResultStreamer extends FileStreamer {
        ...
        public void addBatteryRecord(final double batteryLevel) throws IOException, KeyException {
            ...
            synchronized (this) {
                ...
                stringBuilder.append(System.currentTimeMillis() * 1000000); // milli sec to nano sec
                stringBuilder.append(String.format(Locale.US, " %.2f", batteryLevel));
